---
layout: post
title: Debugging a Race Condition Between Microsoft Edge and SharePoint
description: >-
...
This also works with electron.js apps and chromium based browsers! Super handy!
...
3. sharepoint.com redirects to sharepoint.com/_forms/default.aspx
...
5. [user enters credentials] login to login.microsoft.com
6. redirects you back to default.aspx
7. POST request to default.aspx redirects you to the SharePoint page you were originally navigating to (step 2) based on the cookie that was set at step 4 (cookie: RpsContextCookie)
...
2. In parallel to the user entering credentials, Edge is pre-loading the new tab page, which is querying the recent/frequent SharePoint urls for their logos to display in the new tab page. These queries to SharePoint <span style='color: red'>CORRUPT</span> the `RpsContextCookie` cookie as they also set it's value.
3. (The user finishes entering credentials and logs in) login.microsoft.com redirects back to sharepoint.com/_forms/Default.aspx where the corrupted `RpsContextCookie` is used to redirect the user to the logo of the last SharePoint site that was fetched in phase 2. 
